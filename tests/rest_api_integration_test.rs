...
async fn create_fetch_delete_sobject() -> Result<()> {
    ...
    let account = client.fetch_by_id::<common::Account>("Account", account.id.as_ref().unwrap()).await?;
    assert_eq!("Test Account", account.get_name().unwrap());

    client.delete_sobject("Account", &account.id.clone().unwrap()).await?;

    match client.fetch_by_id::<common::Account>("Account", account.id.as_ref().unwrap()).await {
