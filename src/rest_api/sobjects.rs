...
impl RestApi {
	...
	pub async fn create_sobject<T: Serialize + Debug + SObject + Clone>(&mut self, mut record: T) -> Result<T> {
	...
	pub async fn describe(&mut self, object_name: &str) -> Result<SObjectInfo> {
		let resource_url = format!("{}/sobjects/{}", self.client.base_version_path()?, object_name);
	...
	pub async fn describe_sobject(&mut self, object_name: &str) -> Result<DescribeSObjectResult> {
		let resource_url = format!("{}/sobjects/{}/describe", self.client.base_version_path()?, object_name);
		let response = self.client.get(resource_url, vec![], vec![]).await?;
		handle_json_response(response).await
	}
	...
	pub async fn describe_global_modified(&mut self, since: String) -> Result<DescribeGlobalResponse> {
		let resource_url = format!("{}/sobjects", self.client.base_version_path()?);
		let response = self
			.client
			.get(resource_url, vec![], vec![("If-Modified-Since".to_string(), since)])
			.await?;
		handle_json_response(response).await
	}
	...
	/// # See
	/// <https://developer.salesforce.com/docs/atlas.en-us.api_rest.meta/api_rest/resources_describeGlobal.htm>
	pub async fn describe_global_unmodified(&mut self, since: String) -> Result<DescribeGlobalResponse> {
		let resource_url = format!("{}/sobjects", self.client.base_version_path()?);
		let response = self
			.client
			.get(resource_url, vec![], vec![("If-Unmodified-Since".to_string(), since)])
			.await?;
		handle_json_response(response).await
	}
	...
	pub async fn delete_sobject(&mut self, sobject_name: &str, id: &str) -> Result<()> {
		let resource_url = format!("{}/sobjects/{}/{}", self.client.base_version_path()?, sobject_name, id);
	...
	pub async fn fetch_by_id<T: DeserializeOwned>(
		// fixme rename into sobject_by_id
		&mut self,
		sobject_name: &str,
		id: &str,
	) -> Result<T> {
	...
	pub async fn update_sobject<T: Serialize + Debug>(
		...
		let resource_url = format!("{}/sobjects/{}/{}", self.client.base_version_path()?, object_name, id);
