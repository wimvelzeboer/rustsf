...
mod tests {
	...
	fn test_create_envelope() {
		...
		assert_eq!(
			envelope,
			"<se:Envelope xmlns:se='http://schemas.xmlsoap.org/soap/envelope/'><se:Header/><se:Body><login xmlns='urn:partner.soap.sforce.com'><username>u</username><password>p</password></login></se:Body></se:Envelope>"
		)
	}
	...
	fn test_extract_nested_tag() {
		...
		assert_eq!(result, Some("https://na1.salesforce.com".to_string()));
	}
	...
	fn test_extra_error_from_body() {
		...
		let access_token = extract_xml_tag("sessionId", &decoded_response).unwrap_or_else(|| "".to_string());
